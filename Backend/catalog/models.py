...
class SubscriptionSystem(TimeStampedModel):
    ...
    display_price_currency = models.CharField(max_length=10, default="TZS")
...
class Subscription(TimeStampedModel):
    ...
    payment_currency = models.CharField(max_length=10, default="TZS")
...
class PackageSubscriptionOrder(TimeStampedModel):
    class Status(models.TextChoices):
        PENDING = "pending", "Pending"
        ACTIVE = "active", "Active"
        COMPLETED = "completed", "Completed"
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="package_subscription_orders")
    package_price = models.ForeignKey(PackagePrice, on_delete=models.PROTECT, related_name="package_subscription_orders")
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.PENDING)
    payment_status = models.CharField(max_length=20, choices=PaymentStatus.choices, default=PaymentStatus.PENDING)
    payment_method = models.CharField(max_length=30, blank=True)
    payment_contact = models.CharField(max_length=120, blank=True)
